...
The accessor becomes available on any `xarray.Dataset` after importing `ovro_lwa_portal`:
...
The dataset must contain the required dimensions (`time`, `frequency`, `polarization`,
`l`, `m`) and the `SKY` data variable. An optional `BEAM` variable enables
beam-related functionality.
